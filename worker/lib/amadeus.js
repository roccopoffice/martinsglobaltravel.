import { computeFlightPricing } from './flight-pricing.js';

let cachedToken = null;
let tokenExpires = 0;

function getAmadeusBase(env) {
  return env.AMADEUS_ENV === 'production' ? 'https://api.amadeus.com' : 'https://test.api.amadeus.com';
}

export async function getAmadeusToken(env) {
  const key = env.AMADEUS_API_KEY;
  const secret = env.AMADEUS_API_SECRET;
  if (!key || !secret) {
    return { error: 'Flight search is not configured. Add AMADEUS_API_KEY and AMADEUS_API_SECRET.' };
  }

  const base = getAmadeusBase(env);
...
export async function amadeusFetch(env, path, query) {
  const auth = await getAmadeusToken(env);
